			...
			reader.addEventListener("load",function(event) {
			  ...
			  if (file.type.match('image')) {
				...
				Upload.prototype.doUpload = function () {
					...
					var service_list_id = $(settings.input_field).parent(settings.preview_box).children(settings.service_list_id);
					...
					formData.append('service_list_id', service_list_id.val());
  ...
  error: function (response) {
   ...
   success: function(response){
	   ...
	   if (response.cover != null) {
		 ...
		 var nfo = response.services;
		 for(var t in nfo){
			 console.log(nfo[t].name)
	   if (nfo[t].photos != null) {
			//if ($(".other-photo-main .image-preview").length == 0) {
				var cant_photos = parseInt($("#photo_limit").val()) - parseInt($(".main-image-preview").length)
		 		for (var i = 0; i < cant_photos; i++) {
					loadPreview(i,nfo[t].service_list_id)
		 		}
			//}
			var cont = 1
			$.each(nfo[t].photos,function(i,itemPhoto){
				 $('.other-photo-main .image-preview[load=0]').each(function(j,item){
					 if($(item).attr("photo_number")==i){
						...
						$(item).children(".service-id").val(itemPhoto.service_list_id)
...
function loadPreview(cant_items,service_list_id){
	...
	$('.other-photo-main').append(
		'<div class="column is-one-third item-preview" >'+
			'<div class="image-preview" load="0" photo_number="'+cant_items+'" >'+
				...
				'<input type="hidden" class="service-id" value="' + service_list_id + '">'+
