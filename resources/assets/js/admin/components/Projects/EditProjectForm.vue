    ...
    <form novalidate class="md-layout" @submit.prevent="validateProject">
      <md-card class="md-layout-item md-size-80 md-small-size-100">
        <md-card-header :data-background-color="dataBackgroundColor">
          <h4 class="title">Editar Proyecto : {{ form.name }}</h4>
          <p class="category">Actualizar datos del Proyecto</p>
        </md-card-header>
        <md-card-content>
          <div class="md-layout">
      ...
      <md-snackbar :md-active.sync="projectSaved">Los datos del Proyecto han sido actualizados con exito!</md-snackbar>
    </form>
  </div>
</template>
...
<script>
  ...
  const saveDataProjectUrl = '/adm/projects/update';
      ...
      loadDataProject() {
        ...
        .then((response) => {
          ...
          if (response.data.project !== null) {
            vm.form = { projectId : response.data.project.id , name: response.data.project.name, description: response.data.project.description }
          }else{
