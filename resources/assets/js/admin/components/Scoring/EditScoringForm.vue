    ...
    <form novalidate class="md-layout" @submit.prevent="validateScoring">
      <md-card class="md-layout-item md-size-80 md-small-size-100">
        <md-card-header :data-background-color="dataBackgroundColor">
          <h4 class="title">Editar Scoring : {{ form.name }}</h4>
          <p class="category">Actualizar datos del Scoring</p>
        </md-card-header>
        <md-card-content>
          <div class="md-layout">
      ...
      <md-snackbar :md-active.sync="scoringSaved">Los datos del Scoring han sido actualizados con exito!</md-snackbar>
    </form>
  </div>
</template>
...
<script>
  ...
  const saveDataScoringUrl = '/adm/scoring/update';
  const loadDataScoringUrl = '/adm/scoring';
  ...
  export default {
    ...
    methods: {
      ...
      loadDataScoring() {
        ...
        var url = loadDataScoringUrl + '/' + scoringId
        axios.get(url)
        .then((response) => {
          vm.isLoading = false

          if (response.data.scoring !== null) {
            vm.form = {
              scoringId : response.data.scoring.id,
              name: response.data.scoring.name,
              description: response.data.scoring.description
            }
          }else{
              vm.$router.push('/scoring')
          }
        });
      }
    },
