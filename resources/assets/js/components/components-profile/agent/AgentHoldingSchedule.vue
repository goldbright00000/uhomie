            ...
            <div class="column is-8">
                <div class="columns">
                    <div class="column is-12 line-down">
                        <span>Visitas para agendar de Propiedad Id: {{$route.params.idProperty}}</span>
                    </div>
                </div>
                <div class="columns">
                        ...
                        <article class="media">
                            ...
                            <div class="media-content">
                                <div class="content is-size-7">
                                    ...
                                    <div>
                                        <b>Precio del proyecto desde: </b><span>UF {{ number_format(property.rent) }}</span>
                                    </div>
                                    <div>
                                        <b>Precio del proyecto hasta: </b><span>UF {{ number_format(property.rent_up) }}</span>
                                    </div>
                                    <div>
        ...
        <div class="columns">
            <div class="column is-full">
                <table class="table is-size-7" style="width: 100%">
                    ...
                    <tbody v-if="property.visitors && property.visitors.length > 0">
                        <visitor v-for="item in property.visitors" :key="item.id" v-bind:info="item"></visitor>
                    </tbody>
                    <tbody v-else>
                        <td colspan="5" class="has-text-centered">
                            No hay visitas agendadas para este proyecto
                        </td>
                    </tbody>
                </table>
            </div>
...
<script>
    ...
    import OwnerCore from './AgentCore';
    ...
    export default {
        ...
        computed: {
            ...
            visitors() {
                if(this.schedule_control == null){
                    return this.$parent.info.projects.filter(property => property.id == this.$route.params.idProperty)[0].visitors
                } else {
                    var visitors = this.$parent.info.projects.filter(property => property.id == this.$route.params.idProperty)[0].visitors
        ...
        data() {
            return {
                ...
                imagesDir: imagesDir,
                postulations: [],
                property: this.$parent.info.projects.filter(property => property.id == this.$route.params.idProperty)[0],
