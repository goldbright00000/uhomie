						...
						error: function (response) {
	  				    ...
	  				    success: function(response){
										...
										if ( cover.val() == 0 ) {
											...
											if ( $('.other-photo-main	.item-preview .image-preview[load=1]').length == $('.other-photo-main	.item-preview .image-preview').length ) {
												var cant = $('.other-photo-main	.item-preview .image-preview[load=1]').length + 1
												loadPreview(cant)
												$('.other-photo-main	.item-preview .image-preview[photo_number=' + cant + ']').each(function(i,item){
													$.ajax({
														url: $("#spaces_uri").val(),
														type:'GET',
														dataType:'JSON',
														success: function(response){
															...
															var options_content = $("")
															options_content = '<select style="width:100%;" class="list-spaces" name="space" id="list-spaces-' + cant + '" list="' + cant + '" >'
															...
															$(item).parents(".item-preview").children(".select-spaces").append(options_content)
															$('#list-spaces-' + cant).select2({ minimumResultsForSearch: -1 })
														}
													});
												})
											}
...
(function ($) {
/*$('#modal-text').text(tips[Math.floor(Math.random()*tips.length)]);
$('.modal-tip').addClass('is-active');*/
 var arrSpaces = []
$.ajax({
  url: $("#photo_uri").val(),
	   ...
	   if (response.cover != null) {
		   	...
		   	$('.main-image-preview').children(".image-id").val(response.cover.id)
				...
				$.ajax({
					...
					dataType:'JSON',
					success: function(response_spaces){
						arrSpaces = response_spaces.spaces
							var options_content = $("")
							options_content = '<select style="width:100%;" class="list-spaces" name="space" id="list-spaces-cover"  >'
							options_content += '<option value="" disabled selected>Que espacio representa?</option>'
							for (var j = 0; j < arrSpaces.length; j++) {
			...
			$.each(response.photos,function(i,itemPhoto){
				 $('.other-photo-main .image-preview[load=0]').each(function(j,item){
					 if($(item).attr("photo_number")==i){
						...
						$(item).attr('load',1)
						if ( $('.other-photo-main .item-preview .image-preview[load=0]').length == 0 ) {
						 	var cant = $('.other-photo-main	.item-preview .image-preview[load=1]').length + 1
					 		loadPreview(cant)
						}
					 }
