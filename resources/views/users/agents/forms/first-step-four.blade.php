@extends('layouts.flujo-base', ['navTitle' => '<span class="bold">Paso 1.4:</span> Registro de tu empresa'])
@section('content')
<div class="container" id="agent-first-step-four">
                  ...
                  <form method="POST" action="" enctype="multipart/form-data" id="registration-form">
                    <div class="field">
                        <div class="label-field">
                            <span>¿Deseas usar tu dirección personal como dirección de tu empresa?</span>
                        </div>
                        <div class="control">
                            <label class="radio">
                                <input type="radio" required name="personal_address" id="personal_address" value="1"  {{$user->getAgentCompany() && $user->getAgentCompany()->personal_address ? 'checked' : ''}}>
                                Si
                            </label>
                            <label class="radio">
                                <input type="radio" required name="personal_address" id="personal_address" value="0" {{$user->getAgentCompany() && !$user->getAgentCompany()->personal_address ? 'checked' : ''}}>
                                No
                            </label>
                        </div>
                    </div>
                    @include('components.users.common-forms.location-data.form2', [
                      'cities' => $cities,
                      'entity' => $user,
                      'back_url' => route('users.agents.first-step.three'),
                      'company' => true
                    ])
              </div>
            </div>
