        ...
        <div class="columns">
            <div class="column is-6">
                <div class="membership-info">
                    ...
                    <a href="{{route($route, ['id' => $property_id])}}" class="button is-outlined is-primary">Cambiar</a>
                </div>
                <table class="plans-table">
                    ...
                    <tr>
                        ...
                        <td class="value">
                            @if ($membership->getFeatures()->properties_count)
                                <img src="{{ asset('images/icono_ok_'.strtolower($membership->name).'.png') }}">
                            @else
                            <img src="{{ asset('images/icono_cruz_'.strtolower($membership->name).'.png') }}">
                            @endif
            ...
            <div class="column side-payment is-5 is-offset-1">
                <form class="" action="@route('payments.memberships')" method="post">
                    ...
                    <input type="hidden" name="success_redir" value="@route('profile.owner')#/order/">
                    @else
                    <input type="hidden" name="success_redir" value="@route('profile.owner')">
                    @endif
                    ...
                    @if($membership->name == 'Basic')
                        <img src="/images/icons/logo_completo.png" heigth="100px" style="margin-bottom: 100px;"><br>
                    @endif
                        <input style="display: inline-block" type="submit" name="submit" id="boton_pagar" value="@if($membership->name == 'Basic') FINALIZAR @else PAGAR @endif" class="button is-outlined is-primary">
                    </div>
    ...
    onAuthorize: function(data, actions) {
      return actions.payment.execute().then(function() {
        ...
        .done(function(response){
            window.location.replace("{{ route("profile.owner") }}");
        })
      });
    }
  }, '#paypal-button');
